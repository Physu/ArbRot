...
class LatentPredictHead(BaseModule):
    ...
    def __init__(self, predictor: dict, loss_total_weight=None) -> None:
        super(LatentPredictHead, self).__init__()
        self.predictor = builder_moco.build_neck_moco(predictor)
        if loss_total_weight is not None:
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> dict:
        ...
        if self.loss_total_weight is not None:
            loss = -(pred_norm * target_norm).sum(dim=1).mean() * self.loss_total_weight
        else:
