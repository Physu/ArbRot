...
class RotHead1217(nn.Module):
    """Simplest classifier head, with only one fc layer.
    不采取之前的360类别，采用12个大类，然后再通过回归的方式得到精确的角度

    跟RotHead1216相比，增加rgd和dgr 两个任务，充分利用已有信息(暂未实现)
    """
    ...
    def __init__(self,
        ...
        super(RotHead1217, self).__init__()
    ...
    def forward(self, x):
        ...
        # print(f"dir_res_norm: {dir_res_norm}")
        dir_res_norm = torch.clamp(dir_res_norm, min=-1.0, max=1.0)
        results['dir_res_norm'] = dir_res_norm  # 这个用来计算loss
        # print(f"dir_res_norm_after: {dir_res_norm}")
    ...
    def loss(self, results, rot_labels, loc_labels):
        ...
        losses['location_loss'] = self.location_loss(
            results['loc_score'],
            loc_labels) / (loc_labels.shape[0] + 1e-8)  # 除以总体的label数量，将其和样本数量对齐
        ...
        losses['dir_class_loss'] = self.dir_class_loss(
            results['dir_class'],
            dir_class_target) / (dir_class_target.shape[0] + 1e-8)
            # weight=heading_loss_weights)

        # calculate direction residual loss, SmoothL1
        # 如果是这种处理，那就是让所有的res 都尽力靠近正确res
        max_rot_pred_index = torch.max(results['dir_class'], dim=1)
        masked = torch.zeros(size=results['dir_class'].shape).cuda()
        for i, index in enumerate(max_rot_pred_index.indices):
            masked[i][index] = 1.
        losses['dir_res_loss'] = self.dir_res_loss(
            results['dir_res_norm'] * masked,
            dir_res_target.unsqueeze(-1).repeat(1, int(self.num_dir_bins/2)) * masked) / (dir_class_target.shape[0] + 1e-8)
