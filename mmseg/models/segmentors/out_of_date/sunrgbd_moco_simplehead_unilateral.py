...
class SUNRGBDMOCOSimpleHeadUnilateral(BaseSegmentor):
    """MOCO.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(SUNRGBDMOCOSimpleHeadUnilateral, self).__init__(init_cfg)
    ...
    def _dep_head_forward_train(self, x, gt_semantic_seg, pre_str, img_metas=None):
        """Run forward function and calculate loss for decode head in
        training."""
        losses = dict()
        out = self.dep_head(x)
        loss = self.dep_head.loss(out, gt_semantic_seg, img_metas)

        losses.update(add_prefix(loss, pre_str))
        return losses

    def _img_head_forward_train(self, x, img, pre_str, img_metas=None):
        """Run forward function and calculate loss for decode head in
        training."""
        losses = dict()
        out = self.img_head(x)
        loss = self.img_head.loss(out, img, img_metas)
    ...
    def forward_train(self,
        ...
        depth_q_backbone = self.encoder_q[0](gt_semantic_seg_concat_aug1)
        depth_k_backbone = self.encoder_q[0](gt_semantic_seg_concat_aug2)
        depth_q = self.encoder_q[1](depth_q_backbone)[0]
        depth_q = nn.functional.normalize(depth_q, dim=1)
        ...
        if self.rot_head is not None:
            loss_rot_im_q = self._rot_head_forward_train(im_q_backbone[3], sunrgbd_rotation1, "rgb1")
            loss_rot_im_k = self._rot_head_forward_train(im_k_backbone[3], sunrgbd_rotation2, "rgb2")
            loss_rot_depth_q = self._rot_head_forward_train(depth_q_backbone[3], sunrgbd_rotation1, "png1")
            loss_rot_depth_k = self._rot_head_forward_train(depth_k_backbone[3], sunrgbd_rotation2, "png2")
        ...
        if self.loc_head is not None:
            loss_loc_im_q = self._loc_head_forward_train(im_q_backbone[3], paste_location1, "rgb1")
            loss_loc_im_k = self._loc_head_forward_train(im_k_backbone[3], paste_location2, "rgb2")
            loss_loc_depth_q = self._loc_head_forward_train(depth_q_backbone[3], paste_location1, "png1")
            loss_loc_depth_k = self._loc_head_forward_train(depth_k_backbone[3], paste_location2, "png2")
        ...
        if self.dep_head is not None:
            loss_dep_depth_q = self._img_head_forward_train(depth_q_backbone[3], img_aug1, "rgb1", img_metas)  # 从png恢复rgb
            loss_dep_depth_k = self._img_head_forward_train(depth_k_backbone[3], img_aug2, "rgb2", img_metas)
            loss_dep_im_q = self._dep_head_forward_train(im_q_backbone[3], gt_semantic_seg_aug1, "png1", img_metas)  # 从rgb恢复png
            loss_dep_im_k = self._dep_head_forward_train(im_k_backbone[3], gt_semantic_seg_aug2, "png2", img_metas)
