...
class RotateCircleImgAndHHAArbRotPlus(object):
    ...
    def __init__(self,
        ...
        self.asymmetric_arbrotplus = asymmetric_arbrotplus
    ...
    def __call__(self, results):
        ...
        # results['paste_location1'] = np.array([interval1[0], interval1[1], radius1])  # 重新赋值，为了后面loss 计算
        # results['paste_location2'] = np.array([interval2[0], interval2[0], radius2])
...
@PIPELINES.register_module()
class RotateRectImgAndHHA(object):
    """Resize and rotate the image & seg.、
    此方法实现RotNet方法自监督方法
    图片整体旋转，最多有四个角度选择
    ...
    def __init__(self,
        ...
        self.resize = resize
        self.angle_list = [i * 90 for i in range(4)]
    ...
    def _rotate_img_and_seg(self, results):
        ...
        degree_index1 = random.randint(0, 4)
        degree_index2 = random.randint(0, 4)
        degree1 = self.angle_list[degree_index1]
        degree2 = self.angle_list[degree_index2]
        # else:
        #     degree_index1 = random.randint(0, self.angle)
        ...
        img_resize1 = mmcv.imresize(results['img'], (self.resize, self.resize))
        img_resize2 = mmcv.imresize(results['img'], (self.resize, self.resize))

        seg_resize1 = mmcv.imresize(results['gt_semantic_seg'], (self.resize, self.resize))
        seg_resize2 = mmcv.imresize(results['gt_semantic_seg'], (self.resize, self.resize))
    ...
    def __call__(self, results):
        ...
        # 如果使用圆形旋转九宫格填充
        self._rotate_img_and_seg(results)  # 传入paste_location 是为了local缩放贴回时需要的信息

        img_crop_save1 = mmcv.imwrite(ori_img1, "newback/imgs_segs/ArbRotPlus/test_samples04/0323_img"+'_' +results['img_info']['filename'])
        seg_crop_save1 = mmcv.imwrite(ori_seg1.astype(np.uint8), "newback/imgs_segs/ArbRotPlus/test_samples04/0323_seg"+'_'+results['img_info']['ann']['seg_map'])
        ...
        img_crop_save1 = mmcv.imwrite(results['img_crop_rotated1'].astype(np.uint8),
                                      "newback/imgs_segs/ArbRotPlus/test_samples04/0323_imgcrop1"+'_' + str(
                                          results['sunrgbd_rotation1']) + '_' + results['img_info']['filename'])
        seg_crop_save1 = mmcv.imwrite(results['seg_crop_rotated1'].astype(np.uint8),
                                      "newback/imgs_segs/ArbRotPlus/test_samples04/0323_segcrop1"+'_' + str(
                                          results['sunrgbd_rotation1']) + '_' + '_' + results['img_info']['ann']['seg_map'])

        results['radius1'] = -1.
        results['radius2'] = -1.
        results['paste_location1'] = np.array([-1., -1., -1., -1.])
        results['paste_location2'] = np.array([-1., -1., -1., -1.])
        return results
