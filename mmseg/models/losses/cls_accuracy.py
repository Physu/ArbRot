...
def cls_accuracy_numpy(pred, target, topk=1, thrs=0.):
...
def cls_accuracy_torch(pred, target, topk=1, thrs=0.):
    if isinstance(thrs, Number):
        thrs = (thrs, )
        res_single = True
    elif isinstance(thrs, tuple):
...
def cls_accuracy(pred, target, topk=1, thrs=0.):
    ...
    if isinstance(pred, torch.Tensor) and isinstance(target, torch.Tensor):
        res = cls_accuracy_torch(pred, target, topk, thrs)
    elif isinstance(pred, np.ndarray) and isinstance(target, np.ndarray):
        res = cls_accuracy_numpy(pred, target, topk, thrs)
    else:
        raise TypeError(
...
class cls_Accuracy(nn.Module):
    ...
    def forward(self, pred, target):
        ...
        """
        return cls_accuracy(pred, target, self.topk)
