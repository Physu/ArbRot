...
class MoCoUnetCT(BaseSegmentor):
    """MOCO.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MoCoUnetCT, self).__init__(init_cfg)
        ...
        if ct_neck is not None:
            self.ct_neck = builder_moco.build_neck_moco(ct_neck)
        else:
            self.ct_neck = None

        if ct_bbox_head is not None:
            self.ct_bbox_head = builder_moco.build_head_moco(ct_bbox_head)
        else:
            self.ct_bbox_head = None
    ...
    def forward_train(self,
        ...
        if self.ct_bbox_head is not None and self.ct_neck is not None:
            ct_x = self.ct_neck.forward(backbone_q)
            ct_x = self.ct_bbox_head.forward(ct_x)
            loss_ct = self.ct_bbox_head.loss(ct_x, sunrgbd_rotation1, paste_location1, img_metas, pre_str='ct')
