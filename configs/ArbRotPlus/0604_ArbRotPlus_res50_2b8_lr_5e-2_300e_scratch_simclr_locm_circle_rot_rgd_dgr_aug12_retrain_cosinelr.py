...
work_dir = 'newback/ArbRotPlus/0604_ArbRotPlus_res50_2b8_lr_5e-2_300e_scratch_simclr_locm_circle_rot_rgd_dgr_aug12_retrain_cosinelr'
...
model = dict(
    ...
    neck=dict(
        ...
        out_channels=64,
        with_avg_pool=True),
    simclr_head=dict(type='ContrastiveHead', temperature=0.1, loss_total_weight=10),
    loc_neck=dict(
        type='GenerationNeck',
    ...
    loc_bbox_head=dict(
        ...
        loss_offset=dict(type='L1Loss', loss_weight=0.2),
        loss_circle=dict(with_circle_iou=True, loss_weight=1.),
        loss_weight_all=1.),  # output_flag 是否输出预测的boxes的位置信息
    rot_head=dict(
        type='RotHeadPlus',
        in_channels=256,
        num_rot_bins=24,
...
# optimizer
optimizer = dict(type='SGD', lr=0.05, weight_decay=1e-4, momentum=0.9)
optimizer_config = dict(grad_clip=None)  # grad_clip, coalesce, bucket_size_mb
...
runner = dict(type='IterBasedRunner', max_iters=33032*3)  # 两卡，bs=8，则100epoch iter为33032
