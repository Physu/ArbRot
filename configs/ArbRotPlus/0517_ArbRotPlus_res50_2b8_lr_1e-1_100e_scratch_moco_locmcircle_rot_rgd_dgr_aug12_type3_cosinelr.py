...
work_dir = 'newback/ArbRotPlus/0517_ArbRotPlus_res50_2b8_lr_1e-1_100e_scratch_moco_locmcircle_rot_rgd_dgr_aug12_type3_cosinelr'
...
model = dict(
    ...
    neck=dict(
        ...
        out_channels=64,
        with_avg_pool=True),
    moco_head=dict(type='ContrastiveHead', temperature=0.2, loss_total_weight=0.5*0.2),
    loc_neck=dict(
        type='GenerationNeck',
    ...
    loc_bbox_head=dict(
        ...
        loss_offset=dict(type='L1Loss', loss_weight=0.3),
        loss_circle=dict(with_circle_iou=True, loss_weight=10),
    ),  # with_circle_iou 是否输出预测的boxes的位置信息
    rot_head=dict(
        type='RotHeadPlus',
        in_channels=256,
        num_rot_bins=24,
...
runner = dict(type='IterBasedRunner', max_iters=33032)  # 两卡，bs=8，则100epoch iter为33032
