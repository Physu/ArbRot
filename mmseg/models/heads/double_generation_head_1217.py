...
class DoubleGenerationHead1217(nn.Module):
    ...
    def __init__(self,
        ...
        super(DoubleGenerationHead1217, self).__init__()
        ...
        self.rgd_linear64d = nn.Linear(3 * 8 * 8, 64)  # 输入 841维度，输出64维度，底下还有一个，分别对应rgb和dgr
        ...
        self.dgr_linear64d = nn.Linear(3 * 8 * 8, 64)  # 输入 841维度，输出64维度
    ...
    def rgd_forward(self, x, momentum=False):
        ...
        if not momentum:
            out_conv_trans1 = self.rgd_convs_trans1(x)
            out_conv_trans1 = self.rgd_convs_trans2(out_conv_trans1)  # generate recovery of image
        # 这部分需要产生输出的
        output = self.convs(x)
    ...
    def dgr_forward(self, x, momentum=False):
        ...
        out_conv_trans1 = None
        if not momentum:
            out_conv_trans1 = self.dgr_convs_trans1(x)
            out_conv_trans1 = self.dgr_convs_trans2(out_conv_trans1)
