...
model = dict(
    ...
    rot_and_loc_head=dict(
        ...
        location_loss=dict(
            ...
            reduction='sum',
            # loss_weight=1.0),
            loss_weight=1.),  # 改变1->0.5->1.
        dir_class_loss=dict(
            type='CrossEntropyLoss', reduction='sum', loss_weight=1.), # 改变1->0.5->1.
        dir_res_loss=dict(
            type='SmoothL1Loss', reduction='sum', loss_weight=0.5), #  改变1->1.->0.5
...
runner = dict(type='IterBasedRunner', max_iters=49548+16)  # 300 epoch
...
workflow = [('train', 1)]
gpu_ids = range(0, 1)
work_dir = 'newback/unilateral_moco/1217_sunrgbd_rot_and_loc_unilateral/1219_sunrgbd_rot_and_loc_unilateral_weight_2'