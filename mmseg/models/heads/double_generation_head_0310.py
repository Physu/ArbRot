...
class DoubleGenerationHead0310(nn.Module):
    ...
    def __init__(self,
        ...
        super(DoubleGenerationHead0310, self).__init__()
    ...
    def depth_unnormalize(self, img, mean, std=0):
        ...
        """
        # mean_cuda = mean
        mean_cuda = mean.repeat(img.size(0), 1, self.out_image_size, self.out_image_size)

        img_cuda = torch.mul(img, mean_cuda)
        # img_cuda = img_cuda.to(torch.int32)
        img_cuda = torch.clamp(img_cuda, min=0, max=65536)

        return img_cuda

    def im_unnormalize(self, img, mean, std=0, to_rgb=True):
        """Inplace unnormalize an image with mean and std.
        ...
        Returns:
            ndarray: The normalized image.
        """
        mean = torch.tensor(mean).cuda().clone().detach()
        mean_cuda = torch.cat((mean.repeat(img.size(0), 1, self.out_image_size, self.out_image_size),
                               mean.repeat(img.size(0), 1, self.out_image_size, self.out_image_size),
                               mean.repeat(img.size(0), 1, self.out_image_size, self.out_image_size)), dim=1)
    ...
    def rgd_loss(self, dep_preds, labels, img_metas=None):
        ...
        max_value, resized_labels = self.resize_and_getmax(labels)
        # losses['loss_l1_dep'] = self.l1_loss(dep_preds, labels) * self.l1_loss_weight

        if self.label_norm_cfg is not None:
            dep_predictions = self.depth_unnormalize(dep_preds, max_value)
            dep_predictions = dep_predictions.to(torch.float32)

            targets = resized_labels

        depth_target = targets[0].to(torch.int32).cpu().numpy().astype(np.uint8)
        # ori_img2_save = mmcv.imwrite(depth_target.transpose((1, 2, 0)),
        #                              "newback/imgs_segs/imgs_head/0312depth_targets_" + img_metas[0]['ann_info']['seg_map'])
        ...
        #                              "newback/imgs_segs/imgs_head/0312depth_prediction_" + img_metas[0]['ann_info']['seg_map'])
        ...
        if self.ssim_loss is not None:
            # 窗口设置为3*3
            losses['loss_ssim_rgd'] = self.ssim_loss(dep_predictions, targets) * self.ssim_weght

        if self.l1_loss is not None:
            losses['loss_l1_rgd'] = self.l1_loss(dep_predictions, targets) * self.l1_loss_weight / self.out_image_size**2
    ...
    def dgr_loss(self, img_preds, targets, img_metas=None):
        ...
        if self.img_norm_cfg is not None:
            ...
            # img_targets = targets
            img_targets = F.interpolate(targets.to(torch.float32), size=[64, 64])

            img_target = img_targets[0].to(torch.int32).cpu().numpy()
            a = torch.from_numpy(np.array([img_target[2], img_target[1], img_target[0]]))
            ori_img2_save = mmcv.imwrite(file_path="newback/imgs_segs/imgs_head/0312_targets_" + img_metas[0]['ori_filename'],
            ...
            ori_img2_save = mmcv.imwrite(file_path="newback/imgs_segs/imgs_head/0312_img_prediction_" + img_metas[0]['ori_filename'],
                                         img=a.to(torch.int32).cpu().numpy().transpose((1, 2, 0)))

        if self.ssim_loss is not None:
            losses['loss_ssim_dgr'] = self.ssim_loss(img_predictions, img_targets) * self.ssim_weght
        if self.l1_loss is not None:
            losses['loss_l1_dgr'] = self.l1_loss(img_predictions, img_targets) * self.l1_loss_weight / self.out_image_size**2
    ...
    def resize_and_getmax(self, labels):
        ...
        resized = F.interpolate(labels, size=[64, 64])
