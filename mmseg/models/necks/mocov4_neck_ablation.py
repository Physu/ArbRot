...
class MoCoV4NeckAblation(BaseModule):
    ...
    def __init__(self,
                 ...
                 with_avg_pool=True,
                 init_cfg=None):
        super(MoCoV4NeckAblation, self).__init__(init_cfg)
    ...
    def unet_decoder_output(self,
        ...
        unet_decoder_out = self.double_generation_head.unet_decoder_forward(combination_img_and_gt, momentum)

        return unet_decoder_out
    ...
    def double_generation_head_forward_train_v2(self,
        ...
        if not momentum:  # 判断是来自gradient update or momentum update, 下面执行的是 gradient update
            out_rgd, out_rgd_for_moco_64d = self.double_generation_head.rgd_forward(unet_final[:, 0:unet_final.shape[1]//2], momentum)
            out_dgr, out_dgr_for_moco_64d = self.double_generation_head.dgr_forward(unet_final[:, unet_final.shape[1]//2:], momentum)
            ...
            out_rgd, out_rgd_for_moco_64d = self.double_generation_head.rgd_forward(
                unet_final[:, 0:unet_final.shape[1]//2], momentum)
            out_dgr, out_dgr_for_moco_64d = self.double_generation_head.dgr_forward(
                unet_final[:, unet_final.shape[1]//2:], momentum)
    ...
    def forward(self, x, sunrgbd_rotation1=None, paste_location1=None, img_aug1=None, gt_semantic1=None, img_metas=None, momentum=None):
        ...
        if self.double_generation_head is not None:
            unet_decoder_output = self.unet_decoder_output(
            ...
            unet_final = torch.cat((unet_decoder_output[0:unet_decoder_output.shape[0] // 2], unet_decoder_output[unet_decoder_output.shape[0] // 2:]), dim=1)
        ...
        if self.double_generation_head is not None:
            loss_gradient, q = self.double_generation_head_forward_train_v2(
                unet_final,  # 这个x，应该市tuple, length 为4
                img_aug1,
                gt_semantic1,  # 注意这里，就是batchsize数目，没有concate操作，即未翻倍
                img_metas,
                momentum=momentum
        ...
        if self.loc_head is not None and momentum is None:
            if len(x) == 4:
                x = x[3]
            # backbone = x[3]
            if self.with_avg_pool:
                x = self.avgpool(x)
            loss_loc = self._loc_head_forward_train(x, paste_location1.to(torch.int64), "loc")
