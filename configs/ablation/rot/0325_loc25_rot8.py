...
model = dict(
    ...
    rot_and_loc_head=dict(
        ...
        num_dir_bins=16,  # 12 dir class, 12 dir res
        with_avg_pool=True,
        rot_weight=1.,
        location_loss=dict(
...
runner = dict(type='IterBasedRunner', max_iters=8075)  # train 10335*100(epoch) /16*2gpu
...
lr_config = dict(
    ...
    warmup_iters=125,  # 在初始的500次迭代中学习率逐渐增加
    warmup_ratio=0.0001,  # 起始的学习率
    step=[4845, 7267])  # 在第0.6*total epch 和0.9* total epoch时降低学习率
...
workflow = [('train', 1)]
gpu_ids = range(0, 1)
work_dir = 'newback/ablation/rot/rot8'