...
class MoCoUnetYOLO0412(BaseSegmentor):
    """MOCO.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MoCoUnetYOLO0412, self).__init__(init_cfg)
        ...
        if yolo_neck is not None:
            self.yolo_neck = builder.build_neck(yolo_neck)
        else:
            self.yolo_neck = None

        if yolo_bbox_head is not None:
            self.yolo_bbox_head = builder.build_head(yolo_bbox_head)
        else:
            self.yolo_bbox_head = None
    ...
    def _yolov3_head_forward_train(self, x, loc, rot, img_metas, pre_str):
        """Run forward function and calculate loss for Depth Generation RGB head in
        training."""
        losses = dict()
        out = self.yolo_neck.forward(x)
        out = self.yolo_bbox_head.forward(out)
        loss = self.yolo_bbox_head.loss(out, loc, img_metas)
    ...
    def forward_train(self,
        ...
        if self.yolo_bbox_head is not None and self.yolo_neck is not None:
            loss_yolov3 = self._yolov3_head_forward_train(backbone_q, paste_location1.repeat(2), sunrgbd_rotation1.repeat(2), img_metas, pre_str='yolov3')
            losses.update((loss_yolov3))
