...
class RotHeadPlus(nn.Module):
    """Simplest classifier head, with only one fc layer.
    专门适配 adashare， 这个和loc_head共享了neck部分的权重
    """
    ...
    def __init__(self,
        ...
        super(RotHeadPlus, self).__init__()
        ...
        num_rot_out_channels = num_rot_bins
    ...
    def forward(self, x):
        """Forward.
                ...
                """
        if self.with_avg_pool:  # x [8,2048,32,32]
            assert x.dim() == 4, \
                "Tensor must has 4 dims, got: {}".format(x.dim())
            x = self.avg_pool(x)
        x = x.view(x.size(0), x.size(1), -1)  # x [8,2048,1,1]
        results = {}
        start, end = 0, 0
        # separate branches
        x_rot = x
        rot_pred = self.conv_rot(x_rot).squeeze(-1)
        # decode rotection
        # rot_preds_trans = rot_pred.transpose(2, 1)
        rot_preds_trans = rot_pred.view(rot_pred.size(0), -1)
        end = end + int(self.num_rot_bins / 2)
    ...
    def angle2class(self, angle):
        ...
        shifted_angle = (angle + angle_per_class / 2) % (
                    2 * np.pi)  # 这里为何要加15度，是为了 15, 45, 75, ..., 相当于将角度修正15度,也就是0度其实是cls 0 的中心位置
    ...
    def loss(self, results, rot_labels):
        ...
        rot_res_target = rot_res_target / self.norm_angle  # 归一化到 -0.5, +0.5 之间 ，相对角度
        ...
        # losses['rot_intra_loss'] = self.loss_total_weight * sum(
        #     [torch.abs(restore_rotation[i + clip_location] - restore_rotation[i]) for i in range(clip_location)]) / (
        #                                        360 * clip_location)
