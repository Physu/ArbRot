...
class ASPPHeadSUNRGBD(BaseDecodeHeadSUNRGBD):
    ...
    def __init__(self, dilations=(1, 6, 12, 18), **kwargs):
        super(ASPPHeadSUNRGBD, self).__init__(**kwargs)
    ...
    def forward(self, inputs):
        """Forward function."""
        x = self._transform_inputs(inputs)  # [B,2048,29,29]
        aspp_outs = [
            resize(
                self.image_pool(x),
                size=x.size()[2:],
        ...
        ]  # 这一步，将resnet1c的输出经过avgpooling, 然后conv2d, 再重新整理成需要的尺寸
        aspp_outs.extend(self.aspp_modules(x))  # aspp_modules 会有四个不同dilation的 DCN, 不过输出的尺寸都是一致的，这一步相当于构建pyramid
