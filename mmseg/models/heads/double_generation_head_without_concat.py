...
class DoubleGenerationHeadWithoutConcat(nn.Module):
    ...
    def __init__(self,
        ...
        super(DoubleGenerationHeadWithoutConcat, self).__init__()
    ...
    def rgd_forward(self, x):
        ...
        out_conv_trans1 = self.bn1(out_conv_trans1)
        out_conv_trans1 = self.convs_trans2(out_conv_trans1)
        out_conv_trans1 = self.bn2(out_conv_trans1)
        # 这部分需要产生输出的
        dep_head_out_for_moco_64d = None
        # output = self.convs(x)
    ...
    def dgr_forward(self, x):
        ...
        img_head_out_for_moco_64d = None
        # output = self.convs(x)
        # output_flatten = output.view(output.size(0), -1)  # 将4维压缩到2维
