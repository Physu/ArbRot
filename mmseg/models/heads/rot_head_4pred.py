...
class RotHead4Pred(BaseModule):
    ...
    def __init__(self,
        ...
        super(RotHead4Pred, self).__init__()
    ...
    def loss(self, results, rot_labels):
        ...
        #     (batch_size, self.num_rot_bins))

        # heading_label_one_hot.scatter_(1, rot_labels.unsqueeze(-1), 1)  # 注意这里，第一个1 表示需要修改的维度，第二个表示需要修改数据的index，第三个表示需要插入的数据
        ...
        losses['rot_class_loss'] = self.loss_total_weight * self.rot_class_loss(
            results['rot_class'],
            rot_labels) / (batch_size + 1e-6)
        # cuda_device = results['rot_class'].get_device()  # 下面是为了计算计算模型大小和计算量的一个workaround
        # if cuda_device != -1:
        ...
        #     save_image(torch.cat(((resized_dep_preds[i]*std+mean).unsqueeze(0), (labels[i]*std+mean).unsqueeze(0)), dim=0),
        #                "newback/see_results/img/1009only_rgd_dgr/1009_dep_prediction_" + img_metas[i]['ori_filename'])  # 在这一步，*255，恢复图像信息
        ...
        # rot_res_select = []
        # for rot_cls_index, rot_res in zip(rot_cls_pred_index, rot_res_pred):
        #     rot_res_select.append(rot_res[rot_cls_index])
        # cuda_device = rot_res_pred.get_device()  # 下面是为了计算计算模型大小和计算量的一个workaround
        # if cuda_device != -1:
        #     rot_res_select = torch.tensor(rot_res_select).to(cuda_device)
        # else:
        #     rot_res_select = torch.tensor(rot_res_select).cpu()

        # # rot_res_select = torch.tensor(rot_res_select).to(rot_res_pred.get_device())  # get_device(),cpu为-1
        # restore_angle = self.class2angle(rot_cls_pred_index, rot_res_select * self.norm_angle)
        # restore_rotation = restore_angle / np.pi * 180
        #
        # clip_location = len(restore_rotation) // 2
        #
        # losses['rot_intra_loss'] = self.loss_total_weight * sum([torch.abs(restore_rotation[i+clip_location]-restore_rotation[i]) for i in range(clip_location)])/(360 * clip_location)
