...
data = dict(
    samples_per_gpu=2,
    workers_per_gpu=2,  # 关于这个worker设置：4*gpu_num https://discuss.pytorch.org/t/guidelines-for-assigning-num-workers-to-dataloader/813/3
    train=dict(
...
runner = dict(type='IterBasedRunner', max_iters=64594)  # train5285 10335*100(epoch) /16
...
lr_config = dict(
    ...
    step=[38757, 58135])                  # 在第0.6*total epch 和0.9* total epoch时降低学习率
...
workflow = [('train', 1)]
gpu_ids = range(0, 1)
work_dir = 'newback/unilateral_moco/0113_moco/0113_moco_bsize_8'