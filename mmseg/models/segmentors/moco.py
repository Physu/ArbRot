...
class MoCo(BaseSegmentor):
    ...
    def double_generation_head_forward_train(self,
        ...
        if not momentum:  # 判断是来自gradient update or momentum update, 下面执行的是 gradient update
            ...
            # unet_out_rgd, unet_out_rgd_for_moco_64d = self.double_generation_head.unet_rgd_forward(combination_img_and_gt[0:ptr // 4],
            #                                                                         momentum)
    ...
    def forward_train(self,
        ...
        if self.loc_head is not None:
            loss_loc_im_q = self._loc_head_forward_train(backbone_q[3], torch.cat((paste_location1, paste_location1), dim=0), "im_q")
            losses.update(loss_loc_im_q)

        if self.rot_and_loc_head is not None:
            loss_rot_and_loc_img_q = self._rot_and_loc_head_forward_train(backbone_q[3], torch.cat((paste_location1, paste_location1), dim=0), torch.cat((paste_location1, paste_location1), dim=0), "im_q")
            losses.update(loss_rot_and_loc_img_q)
