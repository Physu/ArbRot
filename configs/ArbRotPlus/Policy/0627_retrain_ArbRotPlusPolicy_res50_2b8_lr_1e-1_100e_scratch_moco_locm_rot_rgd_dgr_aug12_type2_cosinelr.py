...
work_dir = 'newback/ArbRotPlusPolicy/0627_retrain_ArbRotPlusPolicy_res50_2b8_lr_1e-1_100e_scratch_moco_locm_rot_rgd_dgr_aug12_type2_cosinelr'
...
img_norm_cfg = dict(
    ...
    to_rgb=True)
model = dict(
    type='ContraUnetPlusPolicyRetrain',
    queue_len=65536,
    feat_dim=128,
    momentum=0.999,
    backbone=dict(
    ...
    neck=dict(
        ...
        with_avg_pool=True),
    moco_head=dict(
        type='ContrastiveHead', temperature=0.2, loss_total_weight=0.1),
    loc_neck=dict(
        type='GenerationNeck',
    ...
    loc_bbox_head=dict(
        ...
        loss_circle=dict(with_circle_iou=False, loss_weight=10)),  # output_flag 是否输出预测的boxes的位置信息
    rot_head=dict(
        type='RotHeadPlus',
        in_channels=256,
        num_rot_bins=24,
    ...
    depth_generate_rgb_head=dict(
        ...
        # l1_loss_weight=1.0,
        loss_total=True,
        loss_total_weight=0.5*0.1,
        ),
    init_temperature=5.0,
    temperature_decay=0.965,
    work_dir=work_dir,
    policy_path='/data/lhy/Cascade/mmsegmentation/newback/ArbRotPlusPolicy/0624_ArbRotPlusPolicy_res50_2b8_lr_1e-1_100e_scratch_moco_locm_rot_rgd_dgr_aug12_type2_cosinelr/policy.pt'
)
data = dict(
    samples_per_gpu=8,
    workers_per_gpu=4,
...
# optimizer
optimizer = dict(type='SGDPolicy', lr=0.01, weight_decay=1e-4, momentum=0.9)
optimizer_config = dict(type='OptimizerPolicyHook')  # grad_clip, coalesce, bucket_size_mb
...
# curriculum_speed 是
# runner = dict(type='IterBasedRunnerWithEpoch', max_iters=30000*4, curriculum_speed=1000, alpha_speed=200)
runner = dict(type='IterBasedRunnerWithEpochForRetrain', max_iters=32297)
checkpoint_config = dict(interval=16100)
