...
class MoCoUnet0405(BaseSegmentor):
    """MOCO.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MoCoUnet0405, self).__init__(init_cfg)
    ...
    def forward_train(self,
        ...
        if self.generation_neck is not None and (self.rgb_generate_depth_head is not None or self.depth_generate_rgb_head is not None):
            ...
            if self.rgb_generate_depth_head is not None:
                loss_rgd = self._rgd_head_forward_train(unet_output[3][0:split_index], gt_semantic_seg_aug1, pre_str='rgd')
                losses.update(loss_rgd)
            if self.depth_generate_rgb_head is not None:
                loss_dgr = self._dgr_head_forward_train(unet_output[3][split_index:], img_aug1, pre_str='dgr')
