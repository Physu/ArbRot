...
class ContraUnetPlus(BaseSegmentor):
    """MOCO.
    ...
    def __init__(self,
        ...
        if moco_head is not None:
            ...
            self.moco_head = builder_moco.build_head_moco(moco_head)
        else:
            self.backbone_q = builder.build_backbone(backbone)
            self.moco_head = None

        if simclr_head is not None:
            self.backbone_q = builder.build_backbone(backbone)
            self.neck_q = builder_moco.build_neck_moco(neck)
            self.simclr_head = builder_moco.build_head_moco(simclr_head)
        else:
            self.backbone_q = builder.build_backbone(backbone)
            self.simclr_head = None

        if simsiam_head is not None:
            self.backbone_q = builder.build_backbone(backbone)
            self.neck_q = builder_moco.build_neck_moco(neck)
            self.simsiam_head = builder.build_head(simsiam_head)
        else:
            self.backbone_q = builder.build_backbone(backbone)
            self.simsiam_head = None

        if byol_head is not None:
            self.backbone_q = builder.build_backbone(backbone)
            self.neck_q = builder_moco.build_neck_moco(neck)
            self.backbone_k = builder.build_backbone(backbone)
            self.neck_k = builder_moco.build_neck_moco(neck)
            ...
            self.byol_head = builder.build_head(byol_head)
    ...
    def _rgd_head_forward_train(self, x, depth, depth_aug1, img_metas, pre_str):
        """Run forward function and calculate loss for RGB Generation Depth head in
        training."""
        losses = dict()
        out = self.rgb_generate_depth_head.forward(x)
        loss = self.rgb_generate_depth_head.loss(out, depth, depth_aug1, img_metas)

        losses.update(add_prefix(loss, pre_str))
        return losses

    def _dgr_head_forward_train(self, x, rgb, rgb_aug1, img_metas, pre_str):
        """Run forward function and calculate loss for Depth Generation RGB head in
        training."""
        losses = dict()
        out = self.depth_generate_rgb_head.forward(x)
        loss = self.depth_generate_rgb_head.loss(out, rgb, rgb_aug1, img_metas)
    ...
    def forward_train(self,
        ...
        if not (self.moco_head or self.simclr_head or self.simsiam_head or self.byol_head):
            backbone_q = self.backbone_q(img_q_and_depth)
