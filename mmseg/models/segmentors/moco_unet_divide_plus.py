...
class MoCoUnetDividePlus(BaseSegmentor):
    """MOCO.
    ...
    def __init__(self,
                 ...
                 loc_neck=None,
                 loc_bbox_head=None,
                 generation_neck_rgd=None,
                 generation_neck_dgr=None,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MoCoUnetDividePlus, self).__init__(init_cfg)
        ...
        if rot_neck is not None:
            self.rot_neck = builder_moco.build_neck_moco(rot_neck)
        else:
            self.rot_neck = None
        ...
        if generation_neck_rgd is not None:
            self.generation_neck_rgd = builder_moco.build_neck_moco(generation_neck_rgd)
        else:
            self.generation_neck_rgd = None
        ...
        if generation_neck_dgr is not None:
            self.generation_neck_dgr = builder_moco.build_neck_moco(generation_neck_dgr)
        else:
            self.generation_neck_dgr = None
    ...
    def forward_train(self,
        ...
        if self.loc_neck is not None and self.loc_head is not None:
        ...
        if self.rot_neck is not None and self.rot_head is not None:
            rot_neck_x = self.loc_neck(backbone_q)
            if self.rot_head is not None:
                loss_rot = self._rot_head_forward_train(rot_neck_x[3], sunrgbd_rotation1.repeat(2), pre_str='rot')
        ...
        if self.generation_neck_rgd is not None and self.rgb_generate_depth_head is not None:
            unet_output_rgd = self.generation_neck_rgd(backbone_q)
            if self.rgb_generate_depth_head is not None:
                loss_rgd = self._rgd_head_forward_train(unet_output_rgd[3][0:split_index], ori_gt_semantic_seg, img_metas,
                                                        pre_str='rgd')
                losses.update(loss_rgd)

        if self.generation_neck_dgr is not None and self.depth_generate_rgb_head is not None:
            unet_output_dgr = self.generation_neck_dgr(backbone_q)
            if self.depth_generate_rgb_head is not None:
                loss_dgr = self._dgr_head_forward_train(unet_output_dgr[3][split_index:], ori_img, img_metas, pre_str='dgr')
