...
class RGBGenerateDepthHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(RGBGenerateDepthHead, self).__init__()
        ...
        if len(self.conv_channels) > 0:
            self.rgd_convs = self._add_conv_branch(
    ...
    def forward(self, x, momentum=False):
        ...
        if self.with_avg_pool:
            x = self.avg_pool(x)
        depth = self.rgd_convs(x)

        return depth

    def loss(self, dep_preds, labels, labels_aug, img_metas=None):
        '''
        :param dep_preds:
        ...
        if self.loss_total is not None:
            tv_loss = self.TV_loss(dep_preds)
            style_loss = self.style_loss(labels, resized_dep_preds)
            preceptual_loss = self.preceptual_loss(labels, resized_dep_preds)
            valid_loss = self.l1_loss(resized_dep_preds, labels)
        if self.loss_total_weight is not None:
            losses['loss_rgd_total'] = (tv_loss * 0.1 + style_loss * 120 + preceptual_loss * 0.05 + valid_loss * 1) * self.loss_total_weight
        else:

            losses['loss_rgd_total'] = (tv_loss * 0.1 + style_loss * 120 + preceptual_loss * 0.05 + valid_loss * 1)

        cuda_device = dep_preds.get_device()  # 下面是为了计算计算模型大小和计算量的一个workaround
        if cuda_device != -1:
        ...
        for i in range(len(resized_dep_preds)):
            if img_metas[i]['ori_filename'] == '000147.jpg' or img_metas[i]['ori_filename'] == '000148.jpg' or img_metas[i]['ori_filename'] == '000149.jpg' or img_metas[i]['ori_filename'] == '000150.jpg':
                ...
                save_image(torch.cat(((resized_dep_preds[i]*std+mean).unsqueeze(0), (labels[i]*std+mean).unsqueeze(0),(labels_aug[i] * std + mean).unsqueeze(0)), dim=0),
                           "newback/see_results/img/20230706/1009_dep_prediction_" + str(self.numerator) + img_metas[i]['ori_filename'])  # 在这一步，*255，恢复图像信息
                # save_image(labels[i], "newback/imgs_segs/rgd/labels_" + img_metas[i]['ori_filename'])
        ...
        # std = torch.tensor(self.img_norm_cfg['std']).cuda()
        # std = std.reshape([len(std), 1, 1]).repeat(1, 256, 256)
        # rgb_preds_upsample = (F.interpolate(dep_preds, size=(labels.shape[2], labels.shape[3])) * std + mean) * 255
        # imgs_recovery = (labels * std + mean) * 255
        ...
        #         print(f"find it:{img_metas[index]['ori_filename']}")
        #         ori_img2_save = mmcv.imwrite(rgb_pd.transpose((1, 2, 0)),
        #                                      "newback/see_results/dep_simsiam/1002_"+str(self.numerator)+"pre_" + img_metas[index][
        #                                          'ori_filename'])
        #         # ori_img2_save = mmcv.imwrite(img.transpose((1, 2, 0)),
        #         #                              "newback/see_results/dep/1002_"+str(self.numerator)+"dep_" + img_metas[index][
        #         #                                  'ori_filename'])
        #         self.numerator = self.numerator + 1
            # print(img_metas[index]['ann_info']['seg_map'])

            # ori_img2_save = mmcv.imwrite(rgb_pd.transpose((1, 2, 0)),
            #                              "newback/see_results/dep_simsiam/1001_deppre_" + img_metas[index]['ori_filename'])
            # ori_img2_save = mmcv.imwrite(img.transpose((1, 2, 0)),
            #                              "newback/see_results/dep_simsiam/1001_depgt_" + img_metas[index]['ori_filename'])
        return losses
