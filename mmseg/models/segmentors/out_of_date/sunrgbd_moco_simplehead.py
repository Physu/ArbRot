...
class SUNRGBDMOCOSimpleHead(BaseSegmentor):
    """MOCO.
    使用了moco，但是为了尽量保全backbone部分的信息，采用可简单的全连接来做最后的分类操作
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(SUNRGBDMOCOSimpleHead, self).__init__(init_cfg)
    ...
    def forward_train(self,
        ...
        loss_rot_im_q = self._rot_head_forward_train(im_q_backbone[3], sunrgbd_rotation1, "1")
        loss_rot_im_k = self._rot_head_forward_train(im_k_backbone[3], sunrgbd_rotation2, "2")

        # out_loc_im_q = self.loc_head.forwar(im_q_backbone)
        # out_loc_im_k = self.loc_head.forwar(im_k_backbone)
        # loss_loc_im_q = self.loc_head(out_loc_im_q, paste_location1)
        # loss_loc_im_k = self.loc_head(out_loc_im_k, paste_location2)

        loss_loc_im_q = self._loc_head_forward_train(im_q_backbone[3], paste_location1, "1")
        loss_loc_im_k = self._loc_head_forward_train(im_k_backbone[3], paste_location2, "2")

        # out_dep_im_q = self.dep_head.forwar(im_q_backbone)
        # out_dep_im_k = self.dep_head.forwar(im_k_backbone)
        # loss_dep_im_q = self.dep_head(out_dep_im_q, gt_semantic_seg_aug1)
        # loss_dep_im_k = self.dep_head(out_dep_im_k, gt_semantic_seg_aug2)

        loss_dep_im_q = self._dep_head_forward_train(im_q_backbone[3], gt_semantic_seg_aug1, "1")
        loss_dep_im_k = self._dep_head_forward_train(im_k_backbone[3], gt_semantic_seg_aug2, "2")
