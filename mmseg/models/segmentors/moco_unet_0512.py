...
class MoCoUnet0512(BaseSegmentor):
    """MOCO.
    ...
    8.0512 通过实验发现，貌似moco+rot+loc 这种形式，效果更好，故将floatweight提升rot loc的loss 权重
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MoCoUnet0512, self).__init__(init_cfg)
        ...
        # self.encoder_q = nn.Sequential(self.backbone_q, self.neck_q)
        # self.encoder_k = nn.Sequential(self.backbone_k, self.neck_k)
        ...
        if generation_neck is not None:
            self.generation_neck = builder.build_neck(generation_neck)
        else:
    ...
    def _parse_losses(self, losses):
        ...
        weight_for_rot, weight_for_double_head = self._loss_weight()

        for _key_, _value_ in log_vars.items():
            _value_ = _value_ * weight_for_rot if 'rot' in _key_ else _value_
            _value_ = _value_ * weight_for_double_head if 'dgr' in _key_ or 'rgd' in _key_ else _value_
            log_vars[_key_] = _value_
        ...
        log_vars['weight_rot'] = weight_for_rot.item()
        log_vars['weight_double'] = weight_for_double_head.item()
        return loss, log_vars

    def _loss_weight(self):
        '''
        注意这个设置，只适用于resnet101的训练，也就是单卡8张，总共200epoch 的时候
        10000\cdot\left(\frac{-1}{1+e^{-\left(\frac{x}{20000}\ -3.5\right)}}+1\right)  用于降低权重
        10000\cdot\left(\frac{1}{1+e^{-\left(\frac{x}{20000}\ -5.\right)}}+1\right)  用于提升权重
        :return:
        '''
        # a = torch.tensor(-(self.iters/20000 - 3.5)).cuda()  # 对于200epoch的时候
        a = torch.tensor(-(self.iters / 20000 - 5.)).cuda()  # 对于300epoch的时候
        weight_shared = 1/(1 + torch.exp(a))
        # weight_for_rot = 1 - weight_shared
        # weight_for_double_head = 1 + weight_shared
        weight_for_rot = 1 + weight_shared
        weight_for_double_head = 1 - weight_shared
        return weight_for_rot.cuda(), weight_for_double_head.cuda()
    ...
    def forward_train(self,
        ...
        if self.loc_head is not None:
            loss_loc = self._loc_head_forward_train(backbone_q, paste_location1.repeat(2), pre_str='loc')
        ...
        if self.generation_neck is not None and (self.rgb_generate_depth_head is not None or self.depth_generate_rgb_head is not None):
            ...
            if self.rgb_generate_depth_head is not None:
                loss_rgd = self._rgd_head_forward_train(unet_output[3][0:split_index], gt_semantic_seg_aug1, img_metas, pre_str='rgd')
                losses.update(loss_rgd)
            if self.depth_generate_rgb_head is not None:
                loss_dgr = self._dgr_head_forward_train(unet_output[3][split_index:], img_aug1, img_metas, pre_str='dgr')
