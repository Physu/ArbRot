...
class STL10Dataset(Dataset):
    ...
    def __init__(self,
        ...
        # load annotations
        self.img_infos = self.load_annotations(self.split, self.folds)
    ...
    def load_annotations(self, split='train', folds=None):
        ...
        if self.split == 'train':
            train_txt = r'./stl10/train_data_idx_backup.txt'
            with open(train_txt, "w") as f:
                for i in range(len(self.labels)):
                    idx_str = str(i).zfill(6) + '_' + str(self.labels[i])
                    f.write(idx_str + "\n")

                    cv2.imwrite('./stl10/train_images/' + str(i).zfill(6) + '_' + str(self.labels[i]) + '.jpg',
                                self.data[i].transpose(1, 2, 0))
                    cv2.imwrite('./stl10/train_labels/' + str(i).zfill(6) + '_' + str(self.labels[i]) + '.png',
                                np.full((96, 96, 1), self.labels[i]))

        if self.split == 'test':
            train_txt = r'./stl10/test_data_idx_backup.txt'
            with open(train_txt, "w") as f:
                for i in range(len(self.labels)):
                    idx_str = str(i).zfill(6) + '_' + str(self.labels[i])
                    f.write(idx_str + "\n")

                    cv2.imwrite('./stl10/test_images/' + str(i).zfill(6) + '_' + str(self.labels[i]) + '.jpg',
                                self.data[i].transpose(1, 2, 0))
                    cv2.imwrite('./stl10/test_labels/' + str(i).zfill(6) + '_' + str(self.labels[i]) + '.png',
                                np.full((96, 96, 1), self.labels[i]))
