...
model = dict(
    type='SUNRGBDMOCOSimpleHeadUnilateral',
    ...
    ssim_loss=dict(type='DirectionAware_SSIM_Loss'),
    ...
    train=dict(
        ...
        pipeline=[
            ...
            dict(type='RotateCircle',
                 resize=225,
                 angle=1,
                 grid=2,  # 注意 resize//grid 的结果表示分割的距离
...
# optimizer
optimizer = dict(type='SGD', lr=1., weight_decay=0.0001, momentum=0.9)
...
workflow = [('train', 1)]
gpu_ids = range(0, 1)
work_dir = 'newback/1205_moco_unilateral_loc_multigpu/1205_unilateral_grid2_multigpu_lre0'