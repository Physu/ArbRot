...
class SUNRGBDMOCOSimpleHeadUnilateralMultiGPU(BaseSegmentor):
    """MOCO.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(SUNRGBDMOCOSimpleHeadUnilateralMultiGPU, self).__init__(init_cfg)
    ...
    def forward_train(self,
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            gt_semantic_seg_concat_aug2, idx_unshuffle_gt_semantic = self._batch_shuffle_ddp(gt_semantic_seg_concat_aug2)
            ...
            depth_k = self._batch_unshuffle_ddp(depth_k, idx_unshuffle_gt_semantic)
        ...
        if self.dep_head is not None:
            loss_dep_im_q = self._dep_head_forward_train(im_q_backbone[3], gt_semantic_seg_aug1, "png1")
            loss_dep_im_k = self._dep_head_forward_train(im_k_backbone[3], gt_semantic_seg_aug2, "png2")
            loss_dep_depth_q = self._img_head_forward_train(depth_q_backbone[3], img_aug1, "rgb1")
            loss_dep_depth_k = self._img_head_forward_train(depth_k_backbone[3], img_aug2, "rgb2")
