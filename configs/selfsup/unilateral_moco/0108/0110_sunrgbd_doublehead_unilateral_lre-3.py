...
model = dict(
    type='SUNRGBDDoubleHeadUnilateral1222',
    ...
    backbone=dict(
        ...
        frozen_stages=-1),
    contrastive_head=dict(type='ContrastiveHead', temperature=0.2, contrastive_weight=1.),
    ...
    double_generation_head=dict(
        type='DoubleGenerationHead1217',
        in_channels=2048,
        out_channels=3,  # 输出image
        norm_cfg=norm_cfg,
        out_image_size=64,  # 控制 估计的深度和图像尺寸
...
runner = dict(type='IterBasedRunner', max_iters=33032)  # 5285*100(epoch) /16
...
lr_config = dict(
    ...
    step=[19819, 29729])                  # 在第0.6*total epch 和0.9* total epoch时降低学习率
...
workflow = [('train', 1)]
gpu_ids = range(0, 1)
work_dir = 'newback/unilateral_moco/0108_sunrgbd_rot_and_loc_unilateral/0112_synBN_no_rotandloc'