...
class MoCoUnet0903(BaseSegmentor):
    """MOCO.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(MoCoUnet0903, self).__init__(init_cfg)
    ...
    def _rgd_head_forward_train(self, x, depth, depth_aug, img_metas, pre_str):
        """Run forward function and calculate loss for RGB Generation Depth head in
        training.
        depth 表示 未使用arbrot
        depth_aug 表示 使用了arbrot增广之后

        """
        losses = dict()
        out = self.rgb_generate_depth_head.forward(x)
        loss = self.rgb_generate_depth_head.loss(out, depth, depth_aug, img_metas)

        losses.update(add_prefix(loss, pre_str))
        return losses

    def _dgr_head_forward_train(self, x, rgb, rgb_aug, img_metas, pre_str):
        """Run forward function and calculate loss for Depth Generation RGB head in
        training.
        rgb 表示 未使用 arbrot
        rgb_aug 表示使用了 arbrot
        """
        losses = dict()
        out = self.depth_generate_rgb_head.forward(x)
        loss = self.depth_generate_rgb_head.loss(out, rgb, rgb_aug, img_metas)
    ...
    def forward_train(self,
                      ...
                      gt_semantic_seg_aug1=None,
                      gt_semantic_seg_aug2=None,
                      sunrgbd_ratation=None,
        ...
        # cuda_device = img.get_device()  # 下面是为了计算计算模型大小和计算量的一个workaround
        # if cuda_device != -1:
        ...
        # for i in range(len(img_aug1)):
        #     save_image((img_aug1[i]*std+mean).unsqueeze(0),
        #                "newback/imgs_segs/segs/1009_dep_prediction_" + img_metas[i]['ori_filename'])  # 在这一步，*255，恢复图像信息
        ...
        if self.generation_neck is not None and (self.rgb_generate_depth_head is not None or self.depth_generate_rgb_head is not None):
            ...
            if self.rgb_generate_depth_head is not None:
                loss_rgd = self._rgd_head_forward_train(unet_output[3][0:split_index], ori_gt_semantic_seg, gt_semantic_seg_aug1, img_metas, pre_str='rgd')
                losses.update(loss_rgd)
            if self.depth_generate_rgb_head is not None:
                loss_dgr = self._dgr_head_forward_train(unet_output[3][split_index:], ori_img, img_aug1, img_metas, pre_str='dgr')
